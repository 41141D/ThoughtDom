...
def get_profile(username: str, db: Session = Depends(get_db)):
    ...
    timeline: List[Milestone] = [Milestone(label="Joined ThoughtDom", date=user.created_at)]
    if posts:
        first_post = min(posts, key=lambda p: p.created_at)
        timeline.append(Milestone(label="First post", date=first_post.created_at))
    ...
    if helpful_items:
        ...
        timeline.append(Milestone(label="First helpful contribution", date=first_helpful.created_at))
    timeline.sort(key=lambda m: m.date)

    # ---- Undated milestone strip: reputation thresholds actually crossed ----
    reputation_milestones = [
        Milestone(label=f"Reputation {t}+", date=None)
