...
export default function CommentThread({
  ...
  if (tree.length === 0) {
    return (
      <div className="rounded-xl2 border border-dashed border-line bg-surface/50 p-6 text-center text-sm text-muted">
...
function CommentNodeView({
  ...
  return (
    <div
      ...
      style={visualDepth > 0 ? { marginLeft: 14, paddingLeft: 14, borderLeft: "2px solid #31333F" } : undefined}
    >
      <div className="rounded-xl2 border border-line bg-surface p-3.5 transition-colors hover:border-line/80">
        <div className="flex items-center gap-2 mb-2">
          ...
          {node.children.length > 0 && (
            <button
              ...
              className="ml-auto text-xs text-muted hover:text-signal transition-colors flex items-center gap-1"
            >
              <span className={`inline-block transition-transform ${collapsed ? "-rotate-90" : ""}`}>▾</span>
              {collapsed ? `${countDescendants(node)} hidden` : "Collapse"}
            </button>
          )}
        </div>
        ...
        <div className="text-sm text-text/90">{renderMarkdown(node.body)}</div>
