...
export default function PostEditor({
  ...
  return (
    <div className="rounded-xl2 border border-line bg-surface overflow-hidden">
      ...
      >
        {dragActive && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-ink/90 border-2 border-dashed border-signal rounded-b-xl2 pointer-events-none">
            <p className="text-signal font-medium">Drop images to upload</p>
          </div>
        )}
        ...
        {showWrite && (
          <textarea
            ...
            placeholder="Write your idea. Drag images in, paste screenshots, or use the toolbar above. Markdown supported."
            rows={14}
        ...
        {showPreview && (
          <div className="px-4 py-3.5 text-sm min-h-[280px] overflow-y-auto">
            {body.trim() ? (
              renderMarkdown(body)
            ) : (
              <p className="text-muted/60 italic">Nothing to preview yet.</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t border-line text-xs text-muted">
        <span>
          {body.length.toLocaleString()} characters &middot; {wordCount.toLocaleString()} words &middot;{" "}
